...
def issue_token(request):
    if request.method == 'POST':
        ...
        if not identity_instance:
            return render(request, 'issuer/error.html', {'message': "The nric number is not recorded! Make sure all the credentials are correct or contact the administrator."})
        else:
            identity = identity_instance[0].id

        #check whether the user have active token
        token_instance= models.Token.objects.filter(identity_id=identity).filter(status=1)
        if token_instance:
            #return HttpResponse("The nric holder still has an active token!")
            return render(request, 'issuer/error.html', {'message': "The nric holder still has an active token! Make sure all the credentials are correct."})
        ...
        if not record:
            return render(request, 'issuer/error.html', {'message': "The nric holder does not have medical records! Contact the administrator for more information."})
        record=record[0]
...
def inactivate_token(request):
    ...
    if request.method == 'POST':
        ...
        identity_instance = models.Identity.objects.filter(nric=nric_num)
        if not identity_instance:
            return render(request, 'issuer/error.html', {'message': "The nric number is not recorded! Make sure all the credentials are correct."})
        else:
            identity = identity_instance[0].id
        ...
        if not token_instance:
            ...
            return render(request, 'issuer/error.html', {'message': "The nric holder has no active token!"})
        else:
