...
class Identity(models.Model):
    ...
    def __str__(self):
        return "id: %s, nric: %s, fullname: %s, address: %s, phone_num: %s" % (self.id, self.nric, self.fullname,
...
class Role(models.Model):
    ...
    def __str__(self):
        return "name: %s, default: %s, permissions: %s" % (self.name, self.default, self.permissions)
...
class Staff(models.Model):
    ...
    def __str__(self):
        return "id: %s, username: %s, role: %s" % (self.id, self.username, self.role.name)
...
class Cluster(models.Model):
    ...
    def __str__(self):
        return "id: %s, name: %s, status: %s" % (self.id, self.name, "active" if self.status else "inactive")
...
class PositiveCases(models.Model):
    ...
    def __str__(self):
        return "id: %s, identity_id: %s, date_test_positive: %s, is_recovered: %s, staff_id: %s" % \
               (self.id, self.identity, self.date_test_positive, self.is_recovered, self.staff)
...
class CloseContact(models.Model):
    ...
    def __str__(self):
        return "id: %s, identity_id: %s, positivecase: %s, staff_id: %s" % (self.id, self.identity, self.positivecase,
                                                                            self.staff)

class Edge(models.Model):
    id = models.BigAutoField(primary_key=True)
    vertex1_id = models.BigIntegerField()
    vertex1_category = models.CharField(max_length=20, choices=[('positive', 'positive case'), ('contact', 'close contact')])
    vertex2_id = models.BigIntegerField()
    vertex2_category = models.CharField(max_length=20, choices=[('positive', 'positive case'), ('contact', 'close contact')])
    cluster = models.ForeignKey(Cluster, on_delete=models.CASCADE)
    def __str__(self):
        return "id: %s, vertex1_id: %s, vertex1_category: %s, vertex2_id: %s, vertex_category: %s, cluster_id: %s" % \
               (self.id, self.vertex1_id, self.vertex1_category, self.vertex2_id, self.vertex2_category, self.cluster)
