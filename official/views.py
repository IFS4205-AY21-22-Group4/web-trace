...
from .models import Identity, Role, Staff, PositiveCases, Cluster, CloseContact, Edge, Token, Gateway, GatewayRecord
...
def index(request):
    ...
    for cluster in cluster_list:
        ...
        cluster_number_dict[cluster] = {'cases_count': cases_count, 'contacts_count': contacts_count}
    context = {
...
def insert(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if cluster:
                case = PositiveCases(identity=identity, date_test_positive=date_test_positive, is_recovered=is_recovered,
                                     staff=staff, cluster=cluster)
            else:
                case = PositiveCases(identity=identity, date_test_positive=date_test_positive, is_recovered=is_recovered,
                                     staff=staff)
            case.save()
...
def update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('official:confirm', args=(positivecase_id,)))
    else:
        form = UpdateForm()
    context = {
        'form': form,
...
def confirm(request, positivecase_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            return HttpResponseRedirect(reverse('official:edit', args=(positivecase_id,)))
    else:
        form = ConfirmForm()
    context = {
        'form': form,
...
def edit(request, positivecase_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            cluster = form.cleaned_data['cluster']
            if cluster:
                case = PositiveCases(identity=identity, date_test_positive=date_test_positive,
...
def assign(request):
    return HttpResponse("Please assign a tracer to a positive case or close contact here.")
